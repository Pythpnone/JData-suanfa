...
def sku_feat_setup_1():
    dump_path = './cache/sku_feat_setup_1_valid'
    if os.path.exists(dump_path):
        with open(dump_path, 'rb') as f:
            df = pickle.load(f)
    else:
        with open('./cache/feat_rule_1_valid', 'rb') as f:
...
# 对于商品最后一次加购/删除时间
def sku_feat_setup_2(start_date='2016-01-30 00:00:00', end_date='2016-04-16 00:00:00'):
    dump_path = './cache/sku_feat_setup_2_valid'
    if os.path.exists(dump_path):
        with open(dump_path, 'rb') as f:
            df = pickle.load(f)
    else:
...
def sku_feat_setup_3():
    dump_path = './cache/sku_feat_setup_3_valid'
    if os.path.exists(dump_path):
        with open(dump_path, 'rb') as f:
            df = pickle.load(f)
    else:
        dataset = sku_feat_setup_2(start_date='2016-01-30 00:00:00', end_date='2016-04-16 00:00:00')
        ...
        temp_df = temp_df[temp_df['time'] > '2016-04-09 00:00:00'][['user_id', 'sku_id', 'type', 'cate_8_days/7']]
...
def all_sku_info():
    dump_path = './cache/all_sku_info_valid'
    if os.path.exists(dump_path):
        with open(dump_path, 'rb') as f:
            df = pickle.load(f)
    else:
...
def get_sku_model():
    ...
    df.to_csv('./feat/valid_sku_model_feat.csv', index=False)
    return df
