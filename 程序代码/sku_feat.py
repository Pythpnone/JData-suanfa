...
def sku_feat_setup_1():
    dump_path = './cache/sku_feat_setup_1'
    if os.path.exists(dump_path):
        with open(dump_path,'rb') as f:
            df = pickle.load(f)
    else:
        with open('./cache/feat_rule_1','rb') as f:
        ...
        df['attend_tm'] = (df['point_max_tm'] - df['point_min_tm']).apply(lambda t:t.total_seconds()/60/60/24)#用户关注该商品天数
...
#对于商品最后一次加购/删除时间
def sku_feat_setup_2(start_date='2016-01-30 00:00:00',end_date='2016-04-11 00:00:00'):
    dump_path = './cache/sku_feat_setup_2'
    if os.path.exists(dump_path):
        with open(dump_path,'rb') as f:
            df = pickle.load(f)
    else:
...
def sku_feat_setup_3():
    dump_path = './cache/sku_feat_setup_3'
    if os.path.exists(dump_path):
        with open(dump_path,'rb') as f:
            df = pickle.load(f)
    else:
        dataset = sku_feat_setup_2(start_date='2016-01-30 00:00:00',end_date='2016-04-11 00:00:00')
        ...
        temp_df = temp_df[temp_df['time']>'2016-04-04 00:00:00'][['user_id','sku_id','type','cate_8_days/7']]
...
def get_sku_model():
    ...
    df = sku_feat_setup_3()
    df = all_sku_info()
    label_dataset = pd.read_csv('test_dataset.csv')
